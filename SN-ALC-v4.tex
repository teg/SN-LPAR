\documentclass[a4paper,UKenglish]{lipics} % Required by actes
...
\author[1]{Tom Gundersen}
\author[2]{Willem Heijltjes}
\author[1]{Michel Parigot}

\authorrunning{Gundersen, Heijltjes, \& Parigot}

\affil[1]{Laboratoire Preuves, Programmes, Systèmes \\ CNRS \& Universit\'e Paris Diderot \\ {\tt teg@jklm.no, parigot@pps.univ-paris-diderot.fr}}
\affil[2]{University of Bath \\ {\tt w.b.heijltjes@bath.ac.uk} }

\subjclass{F.4.1}

\keywords{lambda-calculus, explicit sharing strong normalisation, reducibility candidates, deep inference}

\Copyright{T.~Gundersen, W.~Heijltjes, and M.~Parigot}
...
\begin{array}{c}
	...
	\\[10pt]
	t^n \quad\coloneq\quad \tup t \mmid t^n\share[m]xu \mmid t^n\distr[m]xy{t^m}
\end{array}
\]
%
where 
...
\noindent
Morever, both simple types and intersection types are preserved in the interpretation of standard lambda-terms as atomic lambda-terms.
...
In this section we prove the strong normalisation theorem for atomic lambda-terms typed in the systems $S_a$ and $D_a$ using Tait's reducibility method. The proof of the main proposition (proposition~\ref{prop:IntSubst}) relies on closure properties of the reducibility sets proved in the next section (section~\ref{sec:ClosPropRedSets}).
...
If $x_1:A_1,\ldots, x_n:A_n \PDA u:B$ and $v_1 \in |A_1|, \dots, v_n \in |A_n|$, then $u\{v_1/x_1, \dots, v_n/x_n\} \in |B|$.
\end{proposition}
...
We proceed by induction on the derivation of $x_1:A_1,\ldots, x_n:A_n \PDA u:B$.
%
...
\\
$((u)v)\{v_1/x_1,\dots,v_n/x_n,w_1/y_1,\dots,w_m/y_m\} = (u\{v_1/x_1,\dots,v_n/x_n\})v\{w_1/y_1,\dots,w_m/y_m\}$. By induction hypothesis, $u\{v_1/x_1,\dots,v_n/x_n\}\in |A\vlim B|$ and $v\{w_1/y_1,\dots,w_m/y_m\}\in |A|$ and therefore $(u\{v_1/x_1,\dots,v_n/x_n\})v\{w_1/y_1,\dots,w_m/y_m\}\in |B|$.
\medskip\\
3) The last rule is 
\[
 \vlinf{}{\typeabs}
...
\]
Let $v_1 \in |C_1|,\dots,v_n \in |C_n|$. We have $(\lambda x.t)\{v_1/x_1,\dots,v_n/x_n\} = \lambda x.t\{v_1/x_1,\dots,v_n/x_n\}$. Let $w\in |A|$. By induction hypothesis we have $t\{v_1/x_1,\dots,v_n/x_n, w/x\}\in |B|$ and therefore $t\{v_1/x_1,\dots,v_n/x_n\}\{ w/x\}\in |B|$. By lemma~\ref{lem:IntCaseLambda}, it follows  $(\lambda x.t\{v_1/x_1,\dots,v_n/x_n\})w \in |B|$. Hence  $(\lambda x.t)\{v_1/x_1,\dots,v_n/x_n\}\in | A\vlim B|$.
\medskip\\
4) The last rule is 
\[
 	  \vliiinf{}{\typeshare}
...
$ s  \{v_1/y_1,\dots,v_k/y_k\}  \{\sn (\lambda y.t')^1/x_1,\dots, \sn (\lambda y.t')^n/x_n\} \in |C|$ and by lemma~\ref{lem:Red-AddSharings}, $ s  \{v_1/y_1,\dots,v_k/y_k\}  \{\sn (\lambda y.t')^1/x_1,\dots, \sn (\lambda y.t')^n/x_n\}[\sh (\lambda y.t')] \in |C|$ . It follows by lemma~\ref{lem:IntCaseSharing} that $ s \{v_1/y_1,\dots,v_k/y_k\}[x_1,\dots, x_n \twoheadleftarrow \lambda y.t'] \in |C|$, i.e. 
\\
$ s\{v_1/y_1,\dots,v_k/y_k\} [x_1,\dots, x_n \twoheadleftarrow \lambda y.u\{w_1/z_1,\dots,w_m/z_m\} ] \in |C|$.
\end{proof}
...
If $x_1:A_1,\dots, x_n:A_n \PDA u:B$ then $u \in \SN$.
\end{theorem}
...
Suppose $x_1:A_1,\dots, x_n:A_n \PDA u:B$. By proposition~\ref{prop:IntBase}, we have $x_1\in |A_1|,\dots, x_n \in|A_n|$. Therefore by proposition~\ref{prop:IntSubst},  $u\{x_1/x_1, \dots, x_n/x_n\} \in |B|$, i.e. $u \in |B|$. Therefore by proposition~\ref{prop:IntBase}, we have $u \in \SN$.

\end{proof}

\noindent
As a consequence of this theorem we get the preservation of the strong normalisation with respect to lambda-calculus (often called PSN property), using the well know fact that the strongly normalisable lambda-terms are precisely the ones typable in $D$ (\cite{Coppo-DezaniCiancaglini-1980,Pottinger-1980,Krivine-1993}). 
...
\begin{proof}
If $N$ is strongly normalisable, then it is typeable in $D$; then by Proposition~\ref{prop:intersection types preserved}, $\tercoden N$ is typeable in $D_a$, and by Theorem~\ref{thm:SN}, $\tercoden N$ is strongly normalisable.
\end{proof}
...
\\
In order to show that $T\in\SN$, we show that each one-step reduction from $T$ gives a term $U\in\SN$. We consider the different possibilities for $U$.
\smallskip
\\
