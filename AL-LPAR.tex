...
%(iii) in $t\share xu$ each $x_i$ must occur in $t$ and becomes bound.
%%
...
%%
%A nullary sharing $t\share*{}u$ is called a \emph{weakening}.
%
%
%
%\begin{ALdefinition}
...
%
The notation $t^*$ indicates either a term or an $n$-term, and $A^*$ either a minimal or conjunctive formula.
%
\emph{Judgments} are of the form $x_1:A_1,\dotsc,x_n:A_n\vdash t^*:B^*$, where $\vv x$ are the free variables of $t^*$. 
%
...
%
It contracts only on $A$, the antecedent of the implications, but not on their consequent. 
%
This may be considered a limited amount of deepness.
...
%
We omit the rules for terms of multiplicity $k$, which can be treated similarly to the rules we do consider.
% and also the rules for intersection types, whose treatment is immediate.
\medskip\\
...
\begin{lemma}\label{lem:rev-preserv-types}
...
\begin{lemma}\label{lem:type-normal-form}
...
\begin{proof}
If $u$ is strongly normalising, then it is strongly normalising with respect to $\ALtype$, so its normal form is typeable with intersection types. By induction on the length of any reduction sequence to the normal form it follows that $u$ is typeable with intersection types by Lemma~\ref{lem:rev-preserv-types}.
\end{proof}
