...
It will be proved that if $T = (u\G[0])\overline{w\G} \in\SN$ where $\term\g\in\SN$ for each closure $\g$ of $\G[0]\overline{\G}$, then $T' = (u\G[0]^*)\overline{w\G^*}\in\SN$.
%
This will be shown by induction on the measure $Mes(T,T')=(\R(T'),m_1(T),m_2(T))$, where
\begin{compactitem}
	\item
 $m_1(T)$ is the sum of the $\R(\term\g)$ for all $\g$ in $\G[0]\overline{\G}$, and
	\item
 $m_2(T)$ is the sum of the $\S(\term\g)$ for all $\g$ in $\G[0]\overline{\G}$.
\end{compactitem}
