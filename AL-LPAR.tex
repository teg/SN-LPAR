...
% If $u\G^*)\overline{w\G*^*} \in \SN$ and for each closure $\g$ of $\G\overline{\G*}$,  $\term (\gamma)\in\SN$, then $(u\G)\overline{w\G*} \in \SN$.
...
It will be proved that if $T = (u)\overline{w\G*} \in\SN$ where $\term\g\in\SN$ for each closure $\g$ of $\overline{\G*}$, then $T' = (u)\overline{w\G*^*}\in\SN$.
%
This will be shown by induction on the measure $(\R(T'),\M(T),\S(T))$, where $\M(T)$ is the sum of the $\R(\term\g)$ for all $\g$ in $\overline{\G*}$.

If,  for each closure $\g$ of $\overline{\G*}$,  $\g^* = \g$, then the result is trivial (note that if $t(\gamma)$ is a variable, then  $\g^* = \g$). We suppose in the following that $\g^* \neq \g$ for at least one$\g$ of $\overline{\G*}$ . We can also suppose that $u$ does not end with a closure, because it can be integrated in the sequence $\G$ and that $\overline{w\G*}$ does not start with an application as that could be integrated in the term $u$.
...
%
For simplicity let $\g$ be the first closure of $\overline{w\G*}$ and name the remainder $\overline{w\G*}'$.
\smallskip
\\
2.1)  $T = (u\share xt)\overline{w\G*}'$ and $U = (u\share x{t'})\overline{w\G*}'$ with $t\rightsquigarrow^1 t'$.
%
In this case $T' = (u\subn{x_i}{\sn(t)^i}[\sh (t)])\overline{w\G*^*}'$.
%
Consider $U' = (u\subn{x_i}{\sn(t')^i}[\sh (t')])\overline{w\G*^*}'$. We have $\sh (t') = \sh (t)$ and by Lemma~\ref{lem:lift-sn}, $\sn (t) \rightsquigarrow \sn (t')$. Therefore $T'\rightsquigarrow U'$.
%
\smallskip
\\
2.2)
$T =  (u\distr xyt)\overline{w\G*}'$ and
$U =  (u\distr xy{t'})\overline{w\G*}'$, with $t ~\rewrite^1~ t'$.
In this case
$T' = (u\subn{x_i}{\sn(\lambda y.\pi_i(t))^i}[\sh(\lambda y.t)])\overline{w\G*^*}'$.
Consider
$U' = (u\subn{x_i}{\sn(\lambda y.\pi_i(t'))^i}[\sh(\lambda y.t')])\overline{w\G*^*}'$.
...
\smallskip
\\
2.3)  $T = (u\share x{(t)v})\overline{w\G*}'$ and
\\
$U = (u\subn{x_i}{(\alpha_i)\beta_i}\share\alpha t\share\beta v)\overline{w\G*}'$.
%
In this case $T' = (u\subn{x_i}{\sn((t)v)^i}[\sh ((t)v)])\overline{w\G*^*}'$.
%
Consider 
\[
...
\begin{tabular}[t]{rclcl}
 $U'$ & $=$ & $(u\subn{x_i}{(\sn (t)^i)\sn (v)^i}[\sh(t)][\sh(v)])\overline{w\G*^*}'$
\\    & $=$ & $(u\subn{x_i}{\sn((t)v)^i}[\sh ((t)v)])\overline{w\G*^*}'$
      & $=$ & $T'$\;.
\end{tabular}\\
%
\smallskip
\\
2.4)  $T = (u\share x{\lambda y.t})\overline{w\G*}'$ and
\\
$U = (u\distr xy{\tup\alpha\share\alpha t})\overline{w\G*}'$.
%
In this case $T' = (u\subn{x_i}{\sn(\lambda y.t)^i}[\sh(\lambda y.t)])\overline{w\G*^*}'$.
%
Consider 
\[
	U' = (u\subn{x_i}{\sn(\lambda y.\alpha\share*\alpha t)^i}[\sh(\lambda y.\alpha[\alpha \leftarrow t])])\overline{w\G*^*}'
\]
%
% NOTE: I made the $\lambda y.\alpha\share*\alpha t$ into variants 
%
We have  $U' = (u\subn{x_i}{\sn(\lambda y.t)^i}[\sh (\lambda y.t)])\overline{w\G*^*}' = T'$.
%\EMPTY{(check this equality)}
%
\smallskip
\\
2.5)  $T = (u\distr xy{\tup t\share*{\vv{\vv*z}}y})\overline{w\G*}'$ and
\\
$U = (u\subn{x_i}{\lambda y_i.t_i\share*{\vv*z_i}{y_i}})\overline{w\G*}'$.
%
In this case we have $T' = ( u\subn{x_i}{\sn(\lambda y_i.t_i\share*{\vv*z_i}y_i)})\overline{w\G*^*}'$.
%
Consider 
\[
	U' = (u\subn{x_i}{\lambda y_i.t_i\share*{\vv*z_i}{y_i}})\overline{w\G*^*}'
...
\medskip
\\
3)  Suppose that U is obtained by the interaction of the two first closures of $\overline{\G*}$ using rule (9). It is enough to consider only the non-trivial case where the term of the second closure is not a variable. We have
%
\begin{align*}
	T &= (u[y_1,\dots,y_m  \leftarrow x_1][x_1,x_2\dots,x_n \leftarrow t])\overline{w\G*}' \\
	U &= (u[y_1,\dots,y_m,x_2\dots,x_n \leftarrow t])\overline{w\G*}'\;.
\end{align*}
%
...
%
\begin{eqnarray*}
 T' & = & (u\share[m]y{x_1}\subn{x_i}{\sn(t)^i}[\sh(t):1,\dotsc,n])\overline{w\G*^*}' \\
    & = & (u\share[m]y{\sn(t)^1}\sub{x_i}{\sn(t)^i}_{2\leq i\leq n}[\sh(t):1,\dotsc,n])\overline{w\G*^*}' \\
    & \rightsquigarrow 
    	& (u\subn[m][j]{y_j}{\sn(t)^{1,j}}[\sh (t^1):(1,1),\dotsc,(1,m)] \\
    &   & \quads4 \sub{x_i}{\sn(t)^i}_{2\leq i\leq n}[\sh (t):1,\dots,n])\overline{w\G*^*}' \\
    & = & (u\subn[m][j]{y_j}{\sn(t)^{1,j}}\sub{x_i}{\sn(t)^i}_{2\leq i\leq n} \\
    &   & \quads4 [\sh (t^1):(1,1),\dots,(1,m)][\sh (t):1,\dots,n])\overline{w\G*^*}'
\end{eqnarray*}
%
...
$
	U' = (u'%\subn[m][j]{y_j}{\sn(t)^{1,j}}\sub{x_i}{\sn(t)^i}_{2\leq i\leq n}
		 [\sh (t):(1,1),\dots,(1,m),2,\dots,n])\overline{w\G*^*}'
$.
