...
\]
is in $\SN$, where $\FV(u')=\{\vv x,\vv[k]z\}$ with $\g$ binding the $z_i$. %$\vv[k]z$.
%
...
\end{gather}
%
Since $\term\g$ is a subterm of $u$, by (ii) above we have the following. %(note a slight abuse of notation, as the variables $\vv x$ 
\[
	B' = ((\tup*{\vv x,\term\g}\G^*)v)\overline{w'\D'^*}\in\SN
\qquad
	\R(B')<\R(T')
\]
Then the induction hypothesis can be applied for $B'$ and the term $B$ below, with $B^n=T^n$, giving $B\in\SN$.
\[
