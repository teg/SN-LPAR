...
%
It will be shown by induction that if $T'\in\SN$ then $T\in\SN$, where
\[
	T = (((\lambda x.u)\G) v)\overline{w\G} \qquad T' = (u\sub xv\G)\overline{w\G}
\]
%
It will be shown that for any term $U$ reached by a reduction step $T\rewrite^1 U$, there is a term $U'$ reached by a reduction $T'\rewrite U'$, such that the induction hypothesis applies to $U$ and $U'$, giving $U\in\SN$.
%
...
For the To each closure $\g$, we associate its \emph{term} $t(\gamma)$ and its \emph{computation} $\g^v$, defined as follows:
\begin{itemize}
