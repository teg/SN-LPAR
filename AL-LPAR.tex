...
%
This will be done by giving a term $U'$ reachable by a reduction $T'\rewrite U'$, to which the induction hypothesis applies; note that since $T'\in\SN$ also $U'\in\SN$, but it must also be shown that the corresponding applicative $n$-term $U^n$ is in $\SN$.
%
...
\qquad
	U'= (u')\overline{w\D^*}
\]
Then $\R(U')<\R(T')$, and since $\FV(u)=\FV(u')$ we have $U^n=T^n\in\SN$.
