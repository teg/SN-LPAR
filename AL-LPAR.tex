...
Here, $\overline{w'\D}$ is $\overline{w\D}$ with the first application $v$ removed; then $ $
	...
	\item
Finally, there is one case where $u=u'\g$, but where a reduction step forces the closure $\g$ into the context $\overline{w\D}$.
%
...
%
To apply the induction hypthesis to $U$ and $U'$, we are forced to include $\g$ into the context $\overline{w\D}$: it must be shown that the $n$-term
\[
