...
%
The measure for the induction is the pair $(S(T'),P(T))$; note that since $T\rewrite U$ and $T'\rewrite U'$, neither component may increase.

The first, special case, is $T'=U$, for which $U\in\SN$ is immediate.
%
For the following cases, $P(U) < P(T)$.

\begin{enumerate}
	\item
If $T\rewrite^1 U$ is an application of permutation rule \eqref{eqn:sharing above application function},
...
\begin{array}{c@{\qquad\qquad}c}
	(((\lambda x.u')[\Gamma]) v')\overline{w'[\Gamma']}
 &  (u'\sub x{v'}[\Gamma])\overline{w'[\Gamma']}
\\[5pt]
	(((\lambda x.u')[\gamma][\Gamma]) v)\overline{w[\Gamma]}
 &	(u'\sub xv[\gamma][\Gamma])\overline{w[\Gamma]}
\\[5pt]
	(((\lambda x.u)[\Gamma']) v)\overline{w[\Gamma]}
 &	(u\sub xv[\Gamma'])\overline{w[\Gamma]}
\end{array}
\]
