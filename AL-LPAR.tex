...
\item
$u=v\share xt$, by the induction hypothesis we have $\Gamma,(x_i:B_i)_{i\leq n}\vdash_D v:A$ and $\Delta\vdash_D t:B$, so $\Gamma,\Delta\vdash_D v\share xt:A$ as in the proof of Proposition~\ref{prop:typable normal form}.
\item
$u=v\distr xyt$, by the induction hypothesis we have $\Gamma, (x_i:B_i\vlim C_i)_{i\leq n}\vdash_D v:A$ and $\Delta,y:X\vdash_D t:B$ so we have $\Delta,y:X^+\vdash_D t:B$ and $\Gamma,\Delta\vdash_D v\distr xyt:A$.
\item
$u=\tup t$, by the induction hypothesis we have, for each $i\leq n$, $\Gamma_i\vdash_D t_i:A_i$, which gives $\Gamma_1,\dots,\Gamma_n\vdash_D\tup t:\vls(A_1.\dots.A_n)$.
\end{itemize}
\end{itemize}
\end{proof}
