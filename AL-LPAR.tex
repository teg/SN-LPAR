...
\item
Given $\Gamma,\Delta \vdash_D (\lambda x.t)\distr*{\vv*y}zv:A$, we have $\Gamma,x:B,\vv*y:(C\vlim D)\vdash_D t:A$ and
$\Delta,z:C \vdash_D v:D$, which gives $\Gamma,\Delta\vdash_D \lambda x.(t\distr*{\vv*y}zv):A$.
\end{itemize}
%\label{eqn:sharing above application function}
%	(u\g)t & ~\ALnotbeta~ ((u)t)\g
\item
...
we have $\Gamma,\vv*y:(C\vlim D)\vdash_D u:B\vlim A$, $\Delta\vdash_D t:B$ and
$\Sigma,z:C \vdash_D v:D$, which gives $\Gamma,\Delta,\Sigma\vdash_D (u\distr*{\vv*y}zv)t:A$.
\end{itemize}
%\label{eqn:sharing above application argument}
%	(u)t\g & ~\ALnotbeta~ ((u)t)\g
\item
...
we have $\Gamma \vdash_D u:B\vlim A$, $\Delta\vv*y:(C\vlim D)\vdash_D t:B$ and
$\Sigma,z:C \vdash_D v:D$, which gives $\Gamma,\Delta,\Sigma\vdash_D (u)t\distr*{\vv*y}zv):A$.
\end{itemize}
%\label{eqn:sharing above sharing}
%	u\share x{t\g} & ~\ALnotbeta~ u\share xt\g
\item
...
$\Sigma \vdash_D v:C$, which gives $\Gamma,\Delta,\Sigma\vdash_D u\share*{\vv*x}{t\share*{\vv*y}v}:A$.
\item
...
we have $\Gamma,\vv*x:B \vdash_D u:A$, $\Delta,\vv*y:(C\vlim D)\vdash_D t:B$ and
$\Sigma,z:C \vdash_D v:D$, which gives $\Gamma,\Delta,\Sigma\vdash_D u\share*{\vv*x}{t\distr*{\vv*y}zv}:A$.
\end{itemize}
%\label{eqn:sharing above distribution}
%	u\distr xy{t\g} & ~\ALnotbeta~ u\distr xyt\g
%		& \text{if}~y\in\FV(t) &&&
...
we have $\Gamma,\vv*x:(E\vlim B)\vdash_D u:A$, $\Delta,\vv*y:C,q:E\vdash_D t:B$ and
$\Sigma \vdash_D v:C$, which gives $\Gamma,\Delta,\Sigma\vdash_D u\distr*{\vv*x}q{t\share*{\vv*y}v}:A$.
\item
...
we have $\Gamma,\vv*x:(E\vlim B) \vdash_D u:A$, $\Delta,\vv*y:(C\vlim D),q:E\vdash_D t:B$ and
$\Sigma,z:C \vdash_D v:D$, which gives $\Gamma,\Delta,\Sigma\vdash_D u\distr*{\vv*x}q{t\distr*{\vv*y}zv}:A$.
\end{itemize}
%\label{eqn:execute unary substitution}
%	u\share*xt~\ALnotbeta~u\sub xt
\item
...
Given $\Gamma,\Delta\vdash_D u\distr*{\vv*x}y{\tup z\share zt}:A$, we have $\Gamma,\vv*x:B\vlim C\vdash_D u:A$ and $\Delta,y:B\vdash_D t:C$, which gives
$\Gamma,\Delta\vdash_D u\share*{\vv*x}{\lambda y.t}:A$.
%\label{eqn:distributor elimination}
%	u\distr xy{\tup t\share*{\vv*z}{y}}~\ALnotbeta~
%	u\subn{x_i}{\lambda y_i.t_i\share*{\vv*z_i}{y_i}}
\item
...
\[
\Gamma, \bigcap_{i\leq n}(x_i:B_i\vlim C_i)_{i\leq n}\vdash_D u:A\;,
\]
...
If a term or $n$-term $t$ is normal with respect to $\ALtype$, it is of one of the following forms, where $u$, $v$ and each $u_i$ are normal.
\[
	x \qquad \lambda x.u \qquad u\share xy \qquad u\share*{}v \qquad (\dotso(x)u_1\dotso )u_n \qquad \tup u
\]
\end{ALproposition}

\begin{ALproposition}
For an atomic lambda-term $t$ in normal form with respect to $\ALtype$ there exist $\Gamma$, $A$ such that $\Gamma\vdash_{D_a} t:A$.
\end{ALproposition}
...
By Proposition~\ref{prop:intersection weakening} $\Gamma,(x_i:B)_{i\leq n}\vdash u:A$, and by the inference rule $(\typeshare)$, for sharing, $\Gamma\vdash u\share xy:A$.
	...
	\item
let $t=u\share*{}v$ with $u$, $v$ normal.
%
By the induction hypothesis there are $\Gamma$, $\Delta$, $A$ and $B$ such that $\Gamma\vdash u:A$ and $\Delta\vdash v:B$
%
By the inference rule $(\typeshare)$, for sharing, $\Gamma,\Delta \vdash u\share*{}v:A$.
...
\begin{proof}
If $u$ is strongly normalising, then it is strongly normalising with respect to $\ALtype$, and its $T$-normal form is typeable with intersection types by Proposition~\ref{prop:normal form}. By induction on the length of any reduction sequence from $u$ to its $T$-normal form it follows that $u$ is typeable with intersection types by Lemma~\ref{lem:reflection}.
\end{proof}
