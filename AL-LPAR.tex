...
% Change \overline\G to \overline\D
...
%%
%We denote by $\overline{w\D}$ a (possibly empty) sequence $w_1[\Gamma_1]\dots w_n[\Gamma_n]$, where $w_1,\dots ,w_n$ are terms (we suppose that a non-empty sequence  $\overline{w\D}$ has at least one term $w_1$).
%By $(u)\overline{w\D}$, we mean $((...(((u)w_1)[\Gamma_1])\dots )w_n)[\Gamma_n]$; thus, each $\G[i]$ may bind not only in $w_i$, but also in $u$, and in all $\G[j]$ and $w_j$ for $j<i$.
%
...
%If $((u\sub xv)\G\G')\overline{w\D} \in \SN$, then $((((\lambda x.u)\G) v)\G')\overline{w\D} \in \SN$.
\end{ALlemma}
...
\end{proof}
%
%For a context $\cxt\overline{w\D}$, the computation $\cxt\overline{w\D}\comp$ is defined inductively below. Note that computation proceeds from inside to outside, since otherwise substitutions would be applied to renamings of previously computed closures.
%%
%\[
%	\cxt(v\overline{w\D})\comp = ((\cxt) v)\overline{w\D}\comp
%\qquad
%	\cxt(\g*\overline{w\D})\comp = (\cxt\g*\comp)\overline{w\D}\comp
%\]
...
%For a sequence of closures $\G = \g[1]\dots\g[p]$, we denote by $\G^*$ a \emph{parallel computation} of the closures in $\G$: a sequence $\g[1]^*\dots\g[p]^*$ such that for any $\g[i]\comp$ and $\g[j]\comp$ in $\G^*$, $\g[i]$ does not bind in $\g[j]$ (and vice versa).
%%
%Then in a term $u\G*$, no $\g[i]\comp$ substitutes inside a $\g[j]\comp$, but only in $u$ and in closures $\g$ in $\G$ that are not computed.
%%
%For a context $\cxt\overline{w\D}$, the parallel computation $\cxt\overline{w\D^*}$ is defined analogously.
%


\begin{ALlemma}\label{lem:IntCaseSharing0}
% If $u\G^*)\overline{w\D^*} \in \SN$ and for each closure $\g$ of $\G\overline{\D}$,  $\term (\gamma)\in\SN$, then $(u\G)\overline{w\D} \in \SN$.
...
\qquad\hbox{and}\qquad
	T'= (u)\overline{w\D^*}\;,
\]
let $T^n$ denote the $n$-term %$\tup*{\vv[k]x,\vv[m]w}\overline\D$, where $\FV(u)=\{\vv[k]x\}$ and $\overline w=\vv[m]w$.
%
...
(Note that in defining $T^n$ we have taken the liberty to use application on $n$-terms, which is strictly not well-defined. This is not a problem since the applications are never part of a redex.)%---if desired, each application $(a)b$ may be replaced by a further tuple $\tup*{a,b}$
...
\qquad
	U'= (u'\sub xv)\overline{w'\D^*}
\]
Here, $\overline{w'\D}$ is $\overline{w\D}$ with the first application $v$ removed; then $ $

	\item
If the reduction step $T\rewrite^1U$ moves a sharing $\g$ from $u$ into the context $\overline{w\D}$, let $u=u'\g$.
%
The context $\overline{w\D}$ consists of closures $\g*$ and applications $(\cdot)v$; to move $\g$ into it, means it must be permuted past a closure $\g*$ or an application $(\cdot)v$.
%
...
%
Accounting for closures $\G$ in $\overline{w\D}$ that may be computed, the rewrite step is of the form $(u'\g\G)v\sim(u'\G\g)v\rewrite^1((u'\G)v)\g$.
%
Then $U$ and $U'$ are as follows, where $\cxt\overline{w\D}=(((\cxt)\G)v)\overline{w'\D'}$.
\[
...
%
To apply the induction hypothesis to $U$ and $U'$, it remains to show that the $n$-term
\[
	U^n = ((\tup x \G)v)\g\overline{w'\D'}
\]
is in $\SN$, where $\FV(u')=\{\vv x\}$.
%
For this we will use the following two facts, where $\FV(\term\g)=\{\vv[m]y\}$:
%
\begin{gather}
\tag{i}
...
\end{gather}
%
Since $\term\g$ is a subterm of $u$, by (ii) above we have the following.
\[
	B' = (((\term\g)\G^*)v)\overline{w'\D'^*}\in\SN
\qquad
	\R(B')<\R(T')
\]
By (i) above we have the following.
\[
	B^n = ((\tup*{\vv[m]y}\G)v)\overline{w'\D'}\in\SN
\]
Then by the induction hypothesis for $B$, given below, and $B'$, we have the following.
\[
	B =  (((\term\g)\G)v)\overline{w'\D'}\in\SN
\]
From this, it follows that $U^n\in\SN$, since $\g$ only binds variables in $\vv x$.


\end{enumerate}
\end{proof}

%The step permutes an individual closure $\g$ past an application (rule \ref{eqn:sharing above application function}) in $\overline{w\D}$. Then $U$ and $U'$ are as follows.
%\[
%	U = (u')\overline{w'\D'}
%\qquad
%	U'= (u')\overline{w'\D'*}
%\]
%	\item
%The step rewrites an individual closure $\g$ to $\g'$, while $\g$ is not computed in $T'$, i.e.\ $\g\in\overline{w\D}$.
%%
...
%
%	\item
%The step applies to an individual closure $\g$ reducing it to $\g'$ with possible substitutions into its context, or permuting it past an application in $\overline{w\D}$.
%%
%In this case, $U$ and $U'$ are as follows, where $\g'$ is computed, i.e.\ $\g'\comp$ is in $\overline{w'\D'*}$.
%\[
%	U = (u')\overline{w'\D'}
%\qquad
%	U'= (u')\overline{w'\D'*}
%\]
%%
%It is easily verified that Computing $\g'$  
%it follows that $T'\rewrite U'$.
%%
%
%
%
%	\item
%The step applies to closure $\g$ that is not computed, i.e.\ $\g$ is in $\overline{w\D^*}$.
%	\item
%The step is an application of 
%
%%
%%\begin{inparaenum}
%%	\item
%%reduction a closure that is computed 
%
%\end{inparaenum}
%
%
...
%	T'= (u\g^*)\overline{w\D^*}
%\]
%let $\G\comp=\g[1]\comp\dots\g[n]\comp$ be the closures in $\g^*\overline{\D^*}$ that are computed, and $\g*[1],\dots,\g*[m]$ those that are not.
%%
%If $T'\in\SN$ and $(\term\g*[i])\G\comp\in\SN$ for each $i\leq m$, then $T\in\SN$.
%
%
%This will be shown by induction on the measure $(\R(T'),\M(T),\S(T))$, where $\M(T)$ is the sum of the $\R(\term\g)$ for all $\g$ in $\overline{\D}$.
%
%If,  for each closure $\g$ of $\overline{\D}$,  $\g^* = \g$, then the result is trivial (note that if $t(\gamma)$ is a variable, then  $\g^* = \g$). We suppose in the following that $\g^* \neq \g$ for at least one$\g$ of $\overline{\D}$ . We can also suppose that $u$ does not end with a closure, because it can be integrated in the sequence $\G$ and that $\overline{w\D}$ does not start with an application as that could be integrated in the term $u$.
%
%SKETCH:
%
%As in previous Lemma we will show that for any $U$, there exists a $U'$, such that 1) the induction hypothesis applies to $U$ and $U'$, and 2) $(\R(U'),\M(U),\S(U))<(\R(T'),\M(T),\S(T))$.
%%
%Notice that if $U$ and $U'$ are on the right from and $T'\rewrite U'$, then 1) follows as $U'\in\SN$ if $T'\in\SN$.
%%
%Moreover, 2) follows from 1):
%%
%First note that $\R(U')\leq\R(T')$ and $\M(U)\leq\M(T)$ as $T\rewrite U$ and $T'\rewrite U'$.
%%
%We then have three cases to consider:
%If $T\rewrite^1 U$ by a sharing reduction, then $\S(U)<\S(T)$, so the measure decreases.
%%
%If $T\rewrite^1 U$ by a reduction in $\term\g$ from $\overline\G$, then $\M(U)<\M(T)$, so the measure decreases.
%%
%Lastly, if $T\rewrite^1 U$ by a non-weakened beta-reduction, then there must be at least one beta-reduction in the reduction path $T\rewrite T'\rewrite U'$, and by definition there can be no beta-reduciton in the path $T\rewrite T'$, so the path $T'\rewrite U'$ can not be empty and $\R(U')<\R(T')$, so the measure decreases.
%
%The only thing that remains to be shown, is that for every $T\rewrite^1 U$ there exists an $U'$ such that $T'\rewrite U'$ and $U$ and $U'$ have the form of the induction hypothesis.
%
%We consider the different possible reductions leading to $U$. If the reductions are inside $u$ or $\overline{w}$ we conclude as in Lemma~\ref{lem:IntCaseLambda0}. If it is the lifting of a closure $\g$ of (or into) $\overline{\D}$ using rule (2) we conclude also as in Lemma~\ref{lem:IntCaseLambda0}. It remains to consider the different possible reductions in a closure $\g$ of $\overline{\D}$ or the interaction of two such closures using rule (9). For simplification we consider the case where the reduction involves $\g[1]$.
%%
%%
%\medskip
%\\
%1) Suppose that $U$ is obtained by the reduction a closure $\g$ such that  $\g^* = \g$. Suppose for instance that is closure $\g[i]$ of $\G = \g[1] \dots\g[p]$, for $\G$ the first closure of $\overline{\D}$. This reduction  may apply substitutions to $u$ and $\gamma_j$, for $j<i$, transforming them into $u^S$ and $\gamma_j^S$. We have in this case $U = (u^S\g[1]^S \dots [\gamma'_i]\dots \g[p])\overline{w\D}$.
%\\
%Consider $U' = (u^S\g[1]^{S*} \dots [\gamma'_i]\dots \g[p]^*)\overline{w\D^*}$ (note that that by $\g[1]^{S*}$ we mean that we apply first the substitution and then computation, and by $\g[1]^{*S}$, the converse).  We have:\quad
%$
%\begin{array}[t]{llll}
% T' & = & (u\g[1]^* \dots\g[p]^*)\overline{w\D^*} & \\
%    & \rightsquigarrow^1 & (u^S\g[1]^{*S} \dots [\gamma'_i]\dots \g[p]^*)\overline{w\D^*} & \\
%    & \rightsquigarrow & (u^S\g[1]^{S*} \dots [\gamma'_i]\dots \g[p]^*)\overline{w\D^*} & \hspace{0.5cm} \mbox{by Lemma~\ref{lem:CompSubst}  }\\
%    & = & U' &
%\end{array}
%$
%\\
...
%%
%For simplicity let $\g$ be the first closure of $\overline{w\D}$ and name the remainder $\overline{w\D}'$.
%\smallskip
%\\
%2.1)  $T = (u\share xt)\overline{w\D}'$ and $U = (u\share x{t'})\overline{w\D}'$ with $t\rightsquigarrow^1 t'$.
%%
%In this case $T' = (u\subn{x_i}{\sn(t)^i}[\sh (t)])\overline{w\D^*}'$.
%%
%Consider $U' = (u\subn{x_i}{\sn(t')^i}[\sh (t')])\overline{w\D^*}'$. We have $\sh (t') = \sh (t)$ and by Lemma~\ref{lem:lift-sn}, $\sn (t) \rightsquigarrow \sn (t')$. Therefore $T'\rightsquigarrow U'$.
%%
%\smallskip
%\\
%2.2)
%$T =  (u\distr xyt)\overline{w\D}'$ and
%$U =  (u\distr xy{t'})\overline{w\D}'$, with $t ~\rewrite^1~ t'$.
%In this case
%$T' = (u\subn{x_i}{\sn(\lambda y.\pi_i(t))^i}[\sh(\lambda y.t)])\overline{w\D^*}'$.
%Consider
%$U' = (u\subn{x_i}{\sn(\lambda y.\pi_i(t'))^i}[\sh(\lambda y.t')])\overline{w\D^*}'$.
%Since $[\sh(\lambda y.t)]=[\sh(\lambda y.t')]$, by Lemma \ref{lem:lift-sn}
%$\sn(\lambda y.\pi_1(t)) \rewrite^\star \sn(\lambda y.\pi_1(t'))$.
%Therefore $T'\rewrite U'$.
%\smallskip
%\\
%2.3)  $T = (u\share x{(t)v})\overline{w\D}'$ and
%\\
%$U = (u\subn{x_i}{(\alpha_i)\beta_i}\share\alpha t\share\beta v)\overline{w\D}'$.
%%
%In this case $T' = (u\subn{x_i}{\sn((t)v)^i}[\sh ((t)v)])\overline{w\D^*}'$.
%%
%Consider 
%\[
...
% $U'$ & $=$ & $(u\subn{x_i}{(\sn (t)^i)\sn (v)^i}[\sh(t)][\sh(v)])\overline{w\D^*}'$
%\\    & $=$ & $(u\subn{x_i}{\sn((t)v)^i}[\sh ((t)v)])\overline{w\D^*}'$
%      & $=$ & $T'$\;.
%\end{tabular}\\
%%
%\smallskip
%\\
%2.4)  $T = (u\share x{\lambda y.t})\overline{w\D}'$ and
%\\
%$U = (u\distr xy{\tup\alpha\share\alpha t})\overline{w\D}'$.
%%
%In this case $T' = (u\subn{x_i}{\sn(\lambda y.t)^i}[\sh(\lambda y.t)])\overline{w\D^*}'$.
%%
%Consider 
%\[
%	U' = (u\subn{x_i}{\sn(\lambda y.\alpha\share*\alpha t)^i}[\sh(\lambda y.\alpha[\alpha \leftarrow t])])\overline{w\D^*}'
%\]
%%
%% NOTE: I made the $\lambda y.\alpha\share*\alpha t$ into variants 
%%
%We have  $U' = (u\subn{x_i}{\sn(\lambda y.t)^i}[\sh (\lambda y.t)])\overline{w\D^*}' = T'$.
%%\EMPTY{(check this equality)}
%%
%\smallskip
%\\
%2.5)  $T = (u\distr xy{\tup t\share*{\vv{\vv*z}}y})\overline{w\D}'$ and
%\\
%$U = (u\subn{x_i}{\lambda y_i.t_i\share*{\vv*z_i}{y_i}})\overline{w\D}'$.
%%
%In this case we have $T' = ( u\subn{x_i}{\sn(\lambda y_i.t_i\share*{\vv*z_i}y_i)})\overline{w\D^*}'$.
%%
%Consider 
%\[
%	U' = (u\subn{x_i}{\lambda y_i.t_i\share*{\vv*z_i}{y_i}})\overline{w\D^*}'
...
%\medskip
%\\
%3)  Suppose that U is obtained by the interaction of the two first closures of $\overline{\D}$ using rule (9). It is enough to consider only the non-trivial case where the term of the second closure is not a variable. We have
%%
%\begin{align*}
%	T &= (u[y_1,\dots,y_m  \leftarrow x_1][x_1,x_2\dots,x_n \leftarrow t])\overline{w\D}' \\
%	U &= (u[y_1,\dots,y_m,x_2\dots,x_n \leftarrow t])\overline{w\D}'\;.
%\end{align*}
%%
...
%%
%\begin{eqnarray*}
% T' & = & (u\share[m]y{x_1}\subn{x_i}{\sn(t)^i}[\sh(t):1,\dotsc,n])\overline{w\D^*}' \\
%    & = & (u\share[m]y{\sn(t)^1}\sub{x_i}{\sn(t)^i}_{2\leq i\leq n}[\sh(t):1,\dotsc,n])\overline{w\D^*}' \\
%    & \rightsquigarrow 
%    	& (u\subn[m][j]{y_j}{\sn(t)^{1,j}}[\sh (t^1):(1,1),\dotsc,(1,m)] \\
%    &   & \quads4 \sub{x_i}{\sn(t)^i}_{2\leq i\leq n}[\sh (t):1,\dots,n])\overline{w\D^*}' \\
%    & = & (u\subn[m][j]{y_j}{\sn(t)^{1,j}}\sub{x_i}{\sn(t)^i}_{2\leq i\leq n} \\
%    &   & \quads4 [\sh (t^1):(1,1),\dots,(1,m)][\sh (t):1,\dots,n])\overline{w\D^*}'
%\end{eqnarray*}
%%
...
%		 [\sh (t):(1,1),\dots,(1,m),2,\dots,n])\overline{w\D^*}'
%$.
%We have $T'\rightsquigarrow U'$.
%%
%
%
...
%
In a second direction, it is expected that the type system and strong normalisation proof can be extended to the second-order case---although subject reduction is not immediately obvious. % in this case.%, where type inference (in e.g.\ a  Hindley--Milner type system) becomes useful
