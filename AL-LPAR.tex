...
%
Its most salient property is that the atomic lambda-calculus implements \emph{fully lazy sharing} \cite{Wadsworth-1971,Hughes-1982,Balabonski-2012}, a degree of sharing that, while standard, had previously been achieved in lambda-calculi only by means of external transformations.
...
The present paper will prove strong normalisation for the typed atomic lambda-calculus using the Tait-reducibility method \cite{Tait-1967,ProofsAndTypes}.
%
...
%
The following mutually recursive grammars simultaneously define the \emph{atomic lambda terms} and the \emph{terms of multiplicity $n$}, or \emph{$n$-terms}, for every $n>0$, written $t^n$. 
%
\setMidspace{10pt}
\[
\begin{array}{@{}r@{}c@{}l@{~}l@{}}
...
%
a sequence of closures will be denoted $\G$, and a sequence $\g[1]\ldots\g[n]$ will be abbreviated $\g[i]_{i\leq n}$.
%
...
%
Standard notions are: $\FV(u)$ is the set of free variables of $u$, and $u\sub xt$ denotes the higher-order substitution of $t$ for $x$ in $u$.
%
...
% and $D_a$
using Tait's reducibility method. The proof of the main proposition (Proposition~\ref{prop:IntSubst}) relies on closure properties of the reducibility sets (Lemma~\ref{lem:IntCaseLambdaSharing}), which again relies on closure properties on the set of strongly normalising atomic lambda trems proved in the next section (Section~\ref{sec:ClosPropSN}).
