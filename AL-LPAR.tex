...
%
To characterise exactly the strongly normalisable terms by an intersection typing discipline proves to be extremely subtle, and a tentative approach is proposed.
\end{abstract}
...
The characterisation of exactly the strongly normalisable atomic lambda-terms by intersection types is more subtle.
%
...
%
A tentative approach to capture these reductions in the type system is explored in the last section of the paper.
...
\begin{inparaenum}[\upshape(i)]
 ...
 \item sharing reductions, denoted $\ALnotbeta$, comprising two kinds of rule: (a) \emph{permutations} taking closures outward (rules~\ref{eqn:sharing above abstraction}--\ref{eqn:sharing above distribution}), and (b) local \emph{transformations} that evaluate closures (rules~\ref{eqn:execute unary substitution}--\ref{eqn:distributor elimination}).
\end{inparaenum}
...
%
The antecedent $x_1\colon A_1,\ldots,x_n : A_n$ of a judgement will be treated as a set, denoted $\Gamma$, $\Delta$, and abbreviated as $(x_i\colon A_i)_{i\leq n}$, or $\vv*x:A$ if $A_i=A$ for all $i$.
...
In this section we show that the  strongly normalising atomic $\lambda$-terms can be characterized as those typeable in a system $D_a$ with intersection types.
%
